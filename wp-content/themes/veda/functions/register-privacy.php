...
if( !class_exists( 'veda_privacy_class' ) ) {

	class veda_privacy_class {
		...
		function __construct() {
			...
			if( isset( $privacy_cmdfrm ) ) {
				add_filter( 'comment_form_defaults', array( $this, 'veda_move_textarea' ) );
				add_action( 'comment_form_top', array( $this, 'veda_move_textarea' ) );

				add_filter( 'comment_form_default_fields', array( $this, 'veda_privacy_comment_checkbox' )  );
				add_filter( 'preprocess_comment', array( $this, 'veda_privacy_verify_comment_checkbox' )  );
			}

			// hook privacy message into mailchimpform
			$privacy_mcfrm = veda_option('privacy','enable-mailchimpfrm-msg');
			if( isset( $privacy_mcfrm ) ) {
				add_filter( 'dt_sc_mailchimp_form_elements', array( $this, 'veda_privacy_mailchimp_checkbox' ) , 10 , 2 );
			}

			

			// hook privacy message into login/registration forms
			$privacy_loginfrm = veda_option('privacy','enable-loginfrm-msg');
			if( isset( $privacy_loginfrm ) ) {
				add_action( 'login_form', array( $this, 'veda_privacy_login_extra' ) , 10 , 2 );
				add_filter( 'wp_authenticate_user', array( $this,'veda_authenticate_user_acc' ), 99999, 2);
			}

			add_action( 'wp_enqueue_scripts', array( $this, 'veda_footer_script' ) , 1000 );
			add_action( 'init', array( $this, 'veda_disable_google_font' ) , 1000 );

			add_action( 'init', array( $this, 'veda_get_tracking_code' ), 999 );
		}

		function veda_move_textarea( $input = array () ) {
			static $textarea = '';
			...
			if ( 'comment_form_defaults' === current_filter() ) {
				...
				$input['label_submit'] = esc_html__('Comment', 'veda');
				$input['title_reply'] = esc_html__( 'Give a Reply', 'veda' );
		 ...
		 * --------------------------------------------------------------------------- */
		function veda_privacy_comment_checkbox( $comment_field = array() ) {
			...
			$comment_field['comment-form-dt-privatepolicy'] = $this->veda_privacy_comment_checkbox_content();
		 ...
		 * --------------------------------------------------------------------------- */
		function veda_privacy_comment_checkbox_content( $content = "", $extra_class = "" ) {

			if( empty($content) ) $content = do_shortcode( veda_option('privacy', 'commentfrm-msg') );
		 ...
		 * --------------------------------------------------------------------------- */
		function veda_privacy_verify_comment_checkbox( $commentdata ) {

		    if ( ! is_user_logged_in() && ! isset( $_POST['comment-form-dt-privatepolicy'] ) ) {
			    $error_message = apply_filters( 'veda_privacy_comment_checkbox_error_message', __( 'Error: You must agree to our privacy policy to comment on this site...' , 'veda' ) );
		 ...
		 * --------------------------------------------------------------------------- */
		function veda_privacy_mailchimp_checkbox( $content = "", $attrs ) {

			if( empty($content) ) $content = do_shortcode( veda_option('privacy', 'mailchimpfrm-msg') );
		...
		/* ---------------------------------------------------------------------------
		 *	Checks if the user accepted the privacy policy in reservation form
		 * --------------------------------------------------------------------------- */
		function veda_privacy_reservation_checkbox( $content = "", $attrs ) {

			if( empty($content) ) 

				$content = do_shortcode( veda_option('privacy', 'reservationfrm-msg') );

			$output = '<div class="dt-privacy-wrapper">';
				$output .= '<input name="dt_reservation_privacy" id="dt_reservation_privacy" type="checkbox" required="required"><label for="dt_reservation_privacy">'.$content.'</label>';
			$output .= '</div>';
		 ...
		 * --------------------------------------------------------------------------- */
		function veda_privacy_login_extra( $form ) {

			$content = do_shortcode( veda_option('privacy', 'loginfrm-msg') );
			echo ( $this->veda_privacy_comment_checkbox_content( $content , 'forgetmenot') );
		}

		/* ---------------------------------------------------------------------------
		 *	Authenticate the extra checkbox in the user login screen
		 * --------------------------------------------------------------------------- */
		function veda_authenticate_user_acc( $user, $password ) {
		    ...
		    if ( isset( $_REQUEST['comment-form-dt-privatepolicy'] ) ) {
		        ...
		        $error = new WP_Error();
		        $error->add('did_not_accept', __( 'You must acknowledge and agree to the privacy policy' , 'veda'));
		        return $error;
		    }
		}
		 ...
		 *	Disable Google Font 
		 * --------------------------------------------------------------------------- */
		function veda_disable_google_font() {
		 ...
		 *	Get Tracking Code 
		 * --------------------------------------------------------------------------- */
		function veda_get_tracking_code() {

			$tracking_code = veda_option('pageoptions', 'tracking-code');

			if( empty( $tracking_code ) ) return;

			$enable_code = veda_option('pageoptions', 'enable-tracking-code');
...
" );
			...
			add_action( 'wp_footer', array( $this, 'veda_print_tracking_code' ), 100 );
		}

		/* ---------------------------------------------------------------------------
		 *	Print Tracking Code
		 * --------------------------------------------------------------------------- */
		function veda_print_tracking_code() {
...
function veda_privacy_helper() {

	return new veda_privacy_class();
}
add_action('init', 'veda_privacy_helper', 20);
 ...
 * -------------------------------------------------------------------------------- */
if( ! function_exists( 'veda_cookie_consent' ) ) {

    function veda_cookie_consent() {

        $cookie_bar = veda_option('privacy','enable-cookie-msgbar');
		if( isset( $cookie_bar ) ) {

			$message  = veda_option('privacy','cookiebar-msg');
			$position = veda_option('privacy','cookiebar-position'); ?>
            ...
            <div class="dt-cookie-consent cookiebar-hidden dt-cookiemessage-<?php echo esc_attr($position); ?>">
	            <div class="container">
					...
					$buttons = veda_option('privacy-bar');
					$i = 0;
					$extra_info = "";

					if( empty( $buttons ) ):
						?><a href="#" class="dt-sc-button filled small dt-cookie-consent-button dt-cookie-close-bar" data-contents="<?php echo esc_attr($cookie_contents); ?>"><?php esc_html_e('OK', 'veda'); ?></a><?php
					endif;
					...
					if( !empty( $buttons ) ):
						foreach($buttons as $button) {
							...
							if( !empty( $button['action'] ) && $button['action'] == 'info_modal' ) {
								...
								$contents[] = array(	'label'		=> __( 'Other external services', 'veda' ), 
				...
				[dt_sc_privacy_video_embeds]', 'veda' )
								);

								$wp_privacy_page = get_option('wp_page_for_privacy_policy');
								if( !empty( $wp_privacy_page ) ) {
									$contents[] = array(	'label'		=> __( 'Privacy Policy', 'veda' ), 
															'content'	=> __( 'You can read about our cookies and privacy settings in detail on our Privacy Policy Page. <br><br> [dt_sc_privacy_link]', 'veda' ));
								}

								$custom_model = veda_option('privacy','enable-custom-model');
								if( isset( $custom_model ) ) {
									$contents = veda_option('privacy-model');
									$heading  = str_replace("'", "&apos;", veda_option('privacy', 'custom-model-title'));
								}
    ...
    add_action('wp_footer', 'veda_cookie_consent', 3);
}