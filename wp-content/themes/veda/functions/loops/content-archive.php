                        ...
                        <?php if( $post_style == "entry-date-left"): ?>
                                ...
                                <div class="entry-details">
                                   ...
                                   <?php if( isset($allow_read_more) && isset($read_more) ):
								   			if( shortcode_exists( 'dt_sc_button' ) && veda_is_plugin_active('js_composer/js_composer.php') ):
												$read_more = stripcslashes($read_more);
												$sc = str_replace("]",' link="url:'.$link.'"]',$read_more);
												$sc = do_shortcode($sc);
												echo !empty( $sc ) ? $sc : '';
											else:
												echo '<a href="'.get_permalink().'" title="'.get_the_title().'" class="dt-sc-button small icon-right with-icon filled type1">'.esc_html__('Read More', 'veda').'<span class="fa fa-long-arrow-right"> </span></a>';
											endif;
