...
jQuery(document).ready(function($){
	...
	if($.cookie("veda_skin")!= null){
		$href = $("link[id='skin-css']").attr("href");
		$href = $href.substr(0,$href.lastIndexOf("/"));
		$href = $href.substr(0,$href.lastIndexOf("/"))+"/"+$.cookie("veda_skin")+"/style.css";
		$("link[id='skin-css']").attr("href",$href);
		$("ul.color-picker a[id='"+$.cookie("veda_skin")+"']").addClass("selected");
	}else{
		$("ul.color-picker a:first").addClass("selected");
	}

	//Apply Layout
	if($.cookie("veda_layout") == "boxed"){
		$("ul.layout-picker li a").removeAttr("class");
		$("ul.layout-picker li a[id='"+$.cookie("veda_layout")+"']").addClass("selected");
		$("div#pattern-holder").removeAttr("style");

		$i = ($.cookie("veda_pattern")) ? $.cookie("veda_pattern")  : 'pattern1';
		...
		$("ul.pattern-picker a[id="+$.cookie("veda_pattern")+"]").addClass('selected');
	} else if($.cookie("veda_layout") == "fullwidth"){
		$("ul.layout-picker li a").removeAttr("class");
		$("ul.layout-picker li a[id='"+$.cookie("veda_layout")+"']").addClass("selected");
		$("div#pattern-holder").removeAttr("style");
	...
	if($.cookie("veda_scheme") != null){
		$("ul.scheme-picker li a").removeAttr("class");
		if( $.cookie("veda_scheme") === "dark" ) {
		...
		else if( $.cookie("veda_scheme") === "light" ) {
	...
	$("ul.layout-picker a").click(function(e){
		...
		$.cookie("veda_layout", $this.attr("id"), { path: '/' });

		if( $.cookie("veda_layout") === "boxed") {
			$("body").removeClass("layout-wide").addClass("layout-boxed");
			$("div#pattern-holder").slideDown();
			
			if( $.cookie("veda_pattern") == null ){
				$("ul.pattern-picker a:first").addClass('selected');
				$.cookie("veda_pattern","pattern1",{ path: '/' });
			}else{
				$("ul.pattern-picker a[id="+$.cookie("veda_pattern")+"]").addClass('selected');
				$img = dttheme_urls.theme_base_url + "/images/patterns/"+$.cookie("veda_pattern")+".jpg";
	...
	$("ul.scheme-picker a").click(function(e){
		...
		$this.addClass("selected");
		$.cookie("veda_scheme", $this.attr("id"), { path: '/' });
		if( $.cookie("veda_scheme") === "dark" ) {
			$("<link id='light-dark-css' href='"+ dttheme_urls.theme_base_url +"/dark/dark-skin.css' rel='stylesheet' media='all' />").insertBefore($('#skin-css'));
		}
		else if( $.cookie("veda_scheme") === "light" ) {
	...
	$("ul.pattern-picker a").click(function(e){
		if($.cookie("veda_layout") == "boxed"){
			...
			$("ul.pattern-picker a").removeAttr("class");
			$this.addClass("selected");
			$.cookie("veda_pattern", $this.attr("id"), { path: '/' });
			$img = dttheme_urls.theme_base_url + "/images/patterns/"+$.cookie("veda_pattern")+".jpg";
			$('body').css('background-image', 'url('+$img+')');
		}
		e.preventDefault();
	...
	$("ul.color-picker a").click(function(e){
		...
		$this.addClass("selected");
		$.cookie("veda_skin", $this.attr("id"), { path: '/' });
		$href = $("link[id='skin-css']").attr("href");
		$href = $href.substr(0,$href.lastIndexOf("/"));
